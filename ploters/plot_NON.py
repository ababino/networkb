...
def plot_NON(NON,colors=['b','r','g','c','m','y','k'],max_n_clus=7):
  ...
  clus_list=[]
  for th in nodes_dic:
    sorted_nodes=sorted([x for x in nodes_dic[th]], key=lambda x: len(x[1]),reverse=True)
  ...
  subNON=nx.subgraph(subNON,nx.connected_components(subNON.to_undirected())[0])
  fig=pl.figure()
  i=0
  for clus in clus_list:
    ...
    nx.draw_networkx_nodes(subNON,pos_clus,nodelist=pos_clus.keys(),node_color=colors[i],node_size=100)
    i=i+1
